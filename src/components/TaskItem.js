...
import {
  ...
  toggleEditTask,
  editTask,
} from "../features/Tasks/tasksListSlice";
...
export const TaskItem = ({
  ...
  const [editTagInput, setEditTagInput] = useState(JSON.stringify({ tagName, tagColor }));
  const [editTitleInput, setEditTitleInput] = useState(title);
  const [editDescrInput, setEditDescrInput] = useState(description);
  ...
  const tagToTagItem = (tag) => {
    ...
    return (
      <option key={tagName} value={JSON.stringify({ tagName, tagColor })}>
        {tagName}
      </option>
    );
  };
      ...
      {!isEdited ? (
        <>
          <div className="card__checkbox">
            <input
              onChange={() => dispatch(completeTask(taskKey))}
              type="checkbox"
              className="card__checkbox--input"
              checked={isCompleted}
              style={{ border: `2px ${tagColor} solid` }}
            />
          </div>
          ...
          <div className="card__controls">
            <div className="card__tag">
              <div
                className="tag__circle"
                onClick={() => handleAddTag("tagTestowy", "blue")}
                style={{ backgroundColor: `${tagColor ? tagColor : "gray"}` }}
              ></div>
            ...
            <div className="card__buttons--container">
              ...
              />
              <RemoveIcon
                onClick={() => dispatch(removeTask(taskKey))}
                className="buttons__button"
              />
            </div>
          </div>
        ...
        <>
          ...
          <div className="card__content">
            ...
            />
            <textarea
              className="card__content--input card__content--editDescription"
              maxLength={DESCRIPTION_MAX_LENGTH}
              value={editDescrInput}
              onChange={(e) => setEditDescrInput(e.target.value)}
            />
          </div>
          <div className="card__controls">
            <div className="card__tag">
              <select
                name="taskTag"
                className="card__content--input card__content--editTag"
                value={editTagInput}
                onChange={(e) => setEditTagInput(e.target.value)}
              >
                <option key="noneOption" value={JSON.stringify({})}>
                  None
                </option>
            ...
            <div className="card__buttons--container">
              ...
              />
              <CheckIcon
                onClick={handlekConfirmEditTask}
                className="buttons__button buttons__button--check"
              />
            </div>
          </div>
        </>
